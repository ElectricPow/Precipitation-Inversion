#!/usr/bin/env bash
# Launch one-node multi-GPU C2-O training. The caller must expose only GPUs
...
STAGE3_MASTER_PORT="${STAGE3_MASTER_PORT:-29741}"
...
echo "[stage3-c2-ddp] physical GPUs: ${CUDA_VISIBLE_DEVICES}"
echo "[stage3-c2-ddp] processes: ${STAGE3_NUM_GPUS}"
echo "[stage3-c2-ddp] rendezvous: ${STAGE3_MASTER_ADDR}:${STAGE3_MASTER_PORT}"
echo "[stage3-c2-ddp] train-only gradient audit runs before the 20-epoch schedule."
...
exec "${PROJECT_ROOT}/.venv/bin/torchrun" \
  ...
  "${PROJECT_ROOT}/scripts/train_stage3_c2.py" \
  --device auto \
  "$@"
